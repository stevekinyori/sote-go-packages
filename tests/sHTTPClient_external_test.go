...
func TestHTTPClientNew(tPtr *testing.T) {
	...
	if _, soteErr := sHTTPClient.New("", TOKEN); soteErr.ErrCode != sError.ErrMissingURL {
...
func TestGet(tPtr *testing.T) {
	...
	if httpm, soteErr = sHTTPClient.New(SURL, TOKEN); soteErr.ErrCode == nil {
		...
		if soteErr = httpm.Get("/get", reqParams, true); soteErr.ErrCode != sError.ErrConversionError {
			tPtr.Errorf("Get failed: Expected error code %v but got %v", sError.ErrConversionError, soteErr)
		}
