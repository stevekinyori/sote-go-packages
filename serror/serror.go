...
var SErrors = map[int]SoteError{
	...
	//
	400000: {400000, ContentError, 2, "", "%v (%v) is not numeric", ""},
	400010: {400010, ContentError, 2, "", "%v (%v) is not a string", ""},
	400020: {400020, ContentError, 2, "", "%v (%v) is not a float", ""},
	400030: {400030, ContentError, 2, "", "%v (%v) is not a array", ""},
	400040: {400040, ContentError, 2, "", "%v (%v) is not a json string", ""},
	400050: {400050, ContentError, 2, "", "%v (%v) is not a valid email address", ""},
	400060: {400060, ContentError, 2, "", "%v (%v) contains special characters which are not allowed", ""},
	400065: {400065, ContentError, 2, "", "%v (%v) contains special characters other than alpha and underscore", ""},
...
func GetSError(code int, params []string) SoteError {
	...
	if fmttdError.ErrCode != code {
		...
		case 5:
			fmttdError.FmtErrMsg = fmt.Sprintf(fmttdError.FmtErrMsg, params[0], params[1], params[2], params[3], params[4])
		default:
